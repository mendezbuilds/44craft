...
/**
 * The homepage teaser's actual selection — admin-flagged via
 * featuredOnHomepage, not "the first N published profiles" (what this
 * used to be before that flag existed). No cap on count by request —
 * every flagged, published member shows, however many that is; ordered
 * by `featuredAt` — when the flag was actually set, not `updatedAt`
 * (which would reshuffle the homepage every time a featured member
 * edited anything unrelated).
 */
